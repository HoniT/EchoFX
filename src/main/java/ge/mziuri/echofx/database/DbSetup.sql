...
CREATE TABLE songs(song_id INT AUTO_INCREMENT PRIMARY KEY, user_id INT, FOREIGN KEY (user_id) REFERENCES users(user_id), title VARCHAR(20), artist VARCHAR(20), album VARCHAR(30), duration DECIMAL(2, 2), address VARCHAR(100));
